(function(){
 ...
 seed();
 function safe(u){return {id:u.id,name:u.name||u.id,role:u.role==="Admin"?"Admin":"User"}}
 window.PRDMSAuth={
  users,saveUsers,
  login(id,password){
 ...
 if(document.body&&!document.body.classList.contains("login-page")){if(!PRDMSAuth.requireLogin())return;document.addEventListener("DOMContentLoaded",()=>{const u=PRDMSAuth.current();document.querySelectorAll("[data-user-name]").forEach(e=>e.textContent=u?.name||"User");document.querySelectorAll("[data-user-role]").forEach(e=>e.textContent=u?.role||"User");});}
})();